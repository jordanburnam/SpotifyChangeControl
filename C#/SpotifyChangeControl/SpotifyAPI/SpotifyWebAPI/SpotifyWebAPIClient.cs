...
{
    public class SpotifyWebAPIClient : IDisposable
    {
        ...
        private PrivateProfile oPrivateProfile;
        private PublicProfile oPublicProfile; 
        ...
        {
            if (this.oPrivateProfile == null) //Get once then cache
            {
                this.oPrivateProfile = DownloadData<PrivateProfile>("https://api.spotify.com/v1/me");
            }
            return this.oPrivateProfile;
        }
        public PublicProfile GetPublicProfileForUser(String userId)
        {
        ...
        /// <summary>
        /// Added by RJB
        /// Used to return only playlists owned by the current user
        /// </summary>
        /// <param name="userId"></param>
        /// <returns></returns>
        public IEnumerable<SimplePlaylist> GetPlaylistsOwnedByUser(String userId = null)
        {
            if (userId == null)
            {
                userId = this.GetPrivateProfile().Id;
            }
            
            List<SimplePlaylist> oSimplePlaylists = new List<SimplePlaylist>();
            Paging<SimplePlaylist> oPagedPlaylists = this.GetUserPlaylists(userId);

            oSimplePlaylists.AddRange(from p in oPagedPlaylists.Items
                                      where p.Owner.Id.Equals(userId)
                                      select p);
            while (oPagedPlaylists.Next != null)
            {

                oPagedPlaylists = DownloadData<Paging<SimplePlaylist>>(oPagedPlaylists.Next);
                oSimplePlaylists.AddRange(from p in oPagedPlaylists.Items
                                          where p.Owner.Id.Equals(userId)
                                          select p);
            ...
            return oSimplePlaylists;
        }


        public Paging<SimplePlaylist> GetUserPlaylists(String userId = null)
        {
            if (userId == null)
            {
                userId = this.GetPrivateProfile().Id;
        ...
        public FullPlaylist GetPlaylist(String playlistId)
        {
            String userId = this.GetPrivateProfile().Id;
        ...
        public Paging<PlaylistTrack> GetPlaylistTracks(String playlistId, String market = "")
        {
            String userId = this.GetPrivateProfile().Id;
            if (market == "")
                return DownloadData<Paging<PlaylistTrack>>("https://api.spotify.com/v1/users/" + userId + "/playlists/" + playlistId + "/tracks");
